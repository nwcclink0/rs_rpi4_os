...
pub struct DeviceDriverDescriptor {
    device_deriver: &'static (dyn interface::DeviceDriver + Sync),
...
impl DriverManager {
   ...
   // Helper for interating over registered drivers.
   fn for_each_descriptor<'a>(&'a self, f: impl FnMut(&'a DeviceDriverDescriptor)) {
    ...
    pub unsafe fn init_drivers(&self) {
        self.for_each_descriptor(|descriptor| {
            //1. Initialize driver.
            if let Err(x) = descriptor.device_deriver.init() {
                panic!(
                    "Error initializing driver: {}: {}",
                    descriptor.device_deriver.compatible(),
                    x
                );
            }
    ...
    pub fn enumerate(&self)  {
        let mut i: usize = 1;
        self.for_each_descriptor(|descriptor| {
            println!("    {}. {}", i, descriptor.device_deriver.compatible());
